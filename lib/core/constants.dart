...
abstract class AppGradients {
  ...
  // Badge gradients
  static const badgeOrange =
      LinearGradient(colors: [Color(0xFFFF8C00), Color(0xFFFFB347)]);
  static const badgeGreen =
      LinearGradient(colors: [Color(0xFF1A8C52), Color(0xFF2ECC71)]);
  static const badgePurple =
      LinearGradient(colors: [Color(0xFF6B3FA0), Color(0xFF9B59B6)]);
  static const badgeBlue =
      LinearGradient(colors: [Color(0xFF1A52A0), Color(0xFF3498DB)]);
  static const badgeRed =
      LinearGradient(colors: [Color(0xFFA01A1A), Color(0xFFE74C3C)]);
  static const badgeTeal =
      LinearGradient(colors: [Color(0xFF1A7A7A), Color(0xFF1ABC9C)]);
}
...
abstract class GameConstants {
  ...
  static double levelProgress(int points) {
    for (int i = 0; i < levelThresholds.length - 1; i++) {
      if (points < levelThresholds[i + 1]) {
        return ((points - levelThresholds[i]) /
  ...
  static int pointsToNextLevel(int points) {
    for (int i = 0; i < levelThresholds.length - 1; i++) {
      if (points < levelThresholds[i + 1]) {
        return levelThresholds[i + 1] - points;
      }
    }
    return 0;
  }
